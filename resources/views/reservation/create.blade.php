...
@section('content')
    <main class="flex-1 overflow-y-auto bg-gray-100 p-6">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Add Reservation</h1>
        ...
        <form action="{{ route('admin.reservations.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="purpose" rows="3"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:ring-amber-500 focus:border-amber-500">{{ old('purpose') }}</textarea>
                @error('purpose')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
